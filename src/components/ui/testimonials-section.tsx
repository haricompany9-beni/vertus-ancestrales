...
const ROW_1_TESTIMONIALS: TestimonialCardData[] = [
  ...
  },
  {
    ...
    name: 'Myriam K.',
    category: 'Rituel beauté',
    review: "Le baume précieux infuse une énergie régénérante incroyable. Vertus Ancestrales n'est pas une simple gamme de cosmétiques, c'est une bénédiction.",
...
const ROW_2_TESTIMONIALS: TestimonialCardData[] = [
  {
    ...
    name: 'Salomé P.',
    category: 'Accompagnement holistique',
    review: "Chaque goutte de sérum résonne comme un mantra de guérison. Mon bien-être est devenu complet, extrêmement vibrant et aligné.",
  ...
  },
  {
    ...
    name: 'Clara S.',
    category: 'Accompagnement holistique',
    review: "Les élixirs et l'huile sacrée m'ont délivrée de mes doutes profonds. Je me sens re-connectée à mon essence originelle et à mes vertus.",
    avatar: 'https://images.unsplash.com/photo-1548142813-c348350df52b?q=80&w=200&auto=format&fit=crop',
    rating: 5
  }
];

// Combine both rows for mobile scrolling list to offer rich variety
const MOBILE_TESTIMONIALS = [...ROW_1_TESTIMONIALS, ...ROW_2_TESTIMONIALS];

export const TestimonialsSection: React.FC = () => {
  return (
    <section id="paroles-sacrees-testimonials" className="bg-[#F8F5F0] py-20 lg:py-28 overflow-hidden relative border-y border-[#E8DFC9]/30">
      ...
      >
        ...
        <p className="text-[13px] sm:text-sm lg:text-base text-gray-500 font-sans max-w-xl mt-3 font-light leading-relaxed">
          Découvrez les retours authentiques de ceux qui ont expérimenté nos rituels, soins et accompagnements.
        </p>
      </motion.div>

      {/* INFINITE SCROLL TRACKS CONTAINER */}
      <div className="w-full flex flex-col gap-8 lg:gap-10 relative z-10">
        
        {/* DESKTOP & TABLET TWO ROWS */}
        <div className="hidden sm:flex flex-col gap-8 lg:gap-10">
          
          {/* ROW 1: Right to Left Scrolling */}
          <div className="w-full overflow-hidden mask-gradient-x select-none">
            {/* Double the list to support seamless infinite loop transitions */}
            <div className="flex gap-6 animate-scroll-left w-max">
              {/* First Cycle */}
              {ROW_1_TESTIMONIALS.map((test) => (
                <TestimonialCard key={`row1-first-${test.id}`} test={test} />
              ))}
              {/* Duplicate Cycle */}
              {ROW_1_TESTIMONIALS.map((test) => (
                <TestimonialCard key={`row1-second-${test.id}`} test={test} />
              ))}
            </div>
          </div>
          ...
          <div className="w-full overflow-hidden mask-gradient-x select-none">
            ...
            <div className="flex gap-6 animate-scroll-right w-max">
              {/* First Cycle */}
              {ROW_2_TESTIMONIALS.map((test) => (
                <TestimonialCard key={`row2-first-${test.id}`} test={test} />
              ))}
              {/* Duplicate Cycle */}
              {ROW_2_TESTIMONIALS.map((test) => (
                <TestimonialCard key={`row2-second-${test.id}`} test={test} />
              ))}
            </div>
          </div>
        ...
        {/* MOBILE SINGLE SCROLL ROW */}
        <div className="sm:hidden w-full overflow-hidden mask-gradient-x select-none">
          <div className="flex gap-4 animate-scroll-left w-max">
            {/* First Cycle */}
            {MOBILE_TESTIMONIALS.map((test) => (
              <TestimonialCard key={`mobile-first-${test.id}`} test={test} isMobile />
            ))}
            {/* Duplicate Cycle */}
            {MOBILE_TESTIMONIALS.map((test) => (
              <TestimonialCard key={`mobile-second-${test.id}`} test={test} isMobile />
            ))}
          </div>
        </div>
...
const TestimonialCard: React.FC<TestimonialCardProps> = ({ test, isMobile = false }) => {
  return (
    <div
    ...
    >
      <div>
        ...
        {/* Review text */}
        <p className={`font-sans font-light italic text-[#3B2F2F]/90 leading-relaxed ${
          isMobile ? 'text-[11px] mb-2 line-clamp-4' : 'text-xs sm:text-[13.5px] line-clamp-4'
        }`}>
          « {test.review} »
        </p>
      </div>
